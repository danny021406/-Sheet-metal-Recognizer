import cv2
import numpy as np
import os
from matplotlib import pyplot as plt
import time

# find the horizontal weight
def horizontal_weight(img,left_x,left_y,h,w):
    #h,w = img.shape[:2]
    h_half = int(h/2)
    w = int(w)
    left_x = int(left_x)
    left_y = int(left_y)
    top_sum = 0
    bot_sum = 0
    rotate = False
    for j in range(h_half):
        for i in range(w):
            x = i+left_x
            top_y = j+left_y
            bot_y = j+left_y+h_half
            p_top = img[top_y,x]
            p_bot = img[bot_y,x]
            top_sum += p_top
            bot_sum += p_bot
    
    if(top_sum > bot_sum):
        rotate = True

    return rotate

# and images
def and_images(img,mask):
    h,w = img.shape[:2]
    out_img = img
    for j in range(h):
        for i in range(w):
            if mask[j,i][1] == 0:
                out_img[j,i] = 0
    
    return out_img

def get_compare_imgs(A,B):
    max_h = max(A.shape[0], B.shape[0])
    max_w = max(A.shape[1], B.shape[1])
    A_img =  np.zeros([max_h,max_w])
    B_img =  np.zeros([max_h,max_w])
    
    for j in range(max_h):
        for i in range(max_w):
            if j < A.shape[0] and i < A.shape[1]:
                A_img[j,i] = A[j,i]
            if j < B.shape[0] and i < B.shape[1]:
                B_img[j,i] = B[j,i]
    
    return A_img, B_img

def rotate_img(image, cx, cy, w, h, angle):
    # grab the dimensions of the image and then determine the
     
    # grab the rotation matrix (applying the negative of the
    # angle to rotate clockwise), then grab the sine and cosine
    # (i.e., the rotation components of the matrix)
    M = cv2.getRotationMatrix2D((cx, cy), angle, 1.0)
    cos = np.abs(M[0, 0])
    sin = np.abs(M[0, 1])
 
    # compute the new bounding dimensions of the image
    nW = int((h * sin) + (w * cos))
    nH = int((h * cos) + (w * sin))
 
    # adjust the rotation matrix to take into account translation
    M[0, 2] += (nW / 2) - cx
    M[1, 2] += (nH / 2) - cy
 
    # perform the actual rotation and return the image
    return cv2.warpAffine(image, M, (nW, nH))

# find contour and rotate the image
def feature_extract(inimg):
    
    # binary input image
    inimg_gray = cv2.cvtColor(inimg, cv2.COLOR_BGR2GRAY)
    ret,img_thd = cv2.threshold(inimg_gray,127,255,cv2.THRESH_BINARY)# binary the gray image
    img_thd = 255-img_thd
    
    img = cv2.cvtColor(img_thd, cv2.COLOR_GRAY2BGR)
    height, width = img.shape[:2]
    # rgb to gray
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    #find contour
    finded = False
    _, contours, hierarchy = cv2.findContours(img_gray,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    for cnt in range(len(contours)):
        #cnt = contours[0]
        rect = cv2.minAreaRect(contours[cnt])
        x, y = rect[0]  # center
        w,h = rect[1]   # width height
        
        contour_size = w*h
        image_size = width*height
        
        print(image_size, contour_size)
        
        if contour_size > image_size/16 and finded == False:
            x,y = rect[0]  # center
            w,h = rect[1]   # width height
            angle = rect[2] # angle
            box = cv2.boxPoints(rect)
            box = np.int0(box)
            
            # if h>w
            if h>w :
                angle -= 90
            
            
            #print('%s    center:(%d,%d),w:%d,h:%d,angle:%d\n'%(name,x,y,w,h,angle))
            #contour_info = '%s    center:(%d,%d),w:%d,h:%d,angle:%d\n'%(name,x,y,w,h,angle)
            #f_feature.write(contour_info)
            
            cv2.drawContours(img,[box], -1, (0, 255, 0), 1) #@ -1(draw all contour) 1:thickness
            #cv2.drawContours(img_gray,[box],-1,(0,0,255),2)
            #cv2.imwrite('test.png', img)
            
            mask = np.zeros(img.shape, dtype="uint8")  # define contour mask
            cv2.drawContours(mask, [box], -1, (255,255,255), -1) # in mask is white
            object_img = and_images(img_gray,mask)     # denoise, get object
            #cv2.imwrite('mask.png', mask)
            
        
            rows, cols = img.shape[:2]
            length = max(rows,cols)*2
            #@ transfer and rotation the object
            s_x = length/2 - x
            s_y = length/2 - y
            print(s_x,s_y)
            H = np.float32([[1,0,s_x],[0,1,s_y]])
            img2 = cv2.warpAffine(object_img, H, (length, length))
            M = cv2.getRotationMatrix2D((length/2, length/2), angle, 1)
            img3 = cv2.warpAffine(img2, M, (length, length))
            
#            name_png2 = outpath + 'denoise_' + name
#            cv2.imwrite(name_png2,img3)
            
            #@ change center to the buttom (rotate 180)
            #get no rotate object
            _2, contours2, hierarchy2 = cv2.findContours(img3,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
            cnt_f = contours2[0]
            for cnt2 in range(len(contours2)):
                rect2 = cv2.minAreaRect(contours2[cnt2])
                w2, h2 = rect2[1]   # width height
                print('123 ',w2,h2)
                if w2*h2 > h*w/2:
                    cnt_f = contours2[cnt2]
                    break

            x2, y2 = rect2[0]  # center
            w2, h2 = rect2[1]   # width height
                
            if h2 > w2: # find long side is width
                t = w2
                w2 = h2
                h2 = t
            
            # print('%s    center:(%d,%d),w:%d,h:%d,length/2:%d\n'%(name,x2,y2,w2,h2,length/2))
#            
            left_x = x2-w2/2
            left_y = y2-h2/2
            rotate180 = horizontal_weight(img3,left_x,left_y,h2,w2)
            if rotate180 == True:
                rotate180 = 180
            else:
                rotate180 = 0
            
            print(rotate180)
            M = cv2.getRotationMatrix2D((length/2, length/2), rotate180, 1)
            img4 = cv2.warpAffine(img3, M, (length, length))
            #name_png2 = outpath + 'hor180_' + name
            #cv2.imwrite(name_png2,img4)
            
            # crop final output
            s_x = int(left_x)+1
            s_y = int(left_y)+1
            e_x = int(s_x+w2)-1
            e_y = int(s_y+h2)-1
            crop_img = img4[s_y:e_y, s_x:e_x]
            #name_png2 = outpath + '0000_' + name
            #cv2.imwrite(name_png2,crop_img)
            
            finded = True
            
            return crop_img
        
##############################################################################
def feature_extract2(inimg):
    
    # binary input image
    inimg_gray = cv2.cvtColor(inimg, cv2.COLOR_BGR2GRAY)
    ret,img_thd = cv2.threshold(inimg_gray,127,255,cv2.THRESH_BINARY)# binary the gray image
    img_thd = 255-img_thd
    
    img = cv2.cvtColor(img_thd, cv2.COLOR_GRAY2BGR)
    height, width = img.shape[:2]
    # rgb to gray
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    #find contour   
    finded = False
    _, contours, hierarchy = cv2.findContours(img_gray,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    for cnt in range(len(contours)):
        #cnt = contours[0]
        rect = cv2.minAreaRect(contours[cnt])
        x, y = rect[0]  # center
        w,h = rect[1]   # width height
        
        contour_size = w*h
        image_size = width*height
        
        print(image_size, contour_size)
        
        if contour_size > image_size/16 and finded == False:
            x,y = rect[0]  # center
            w,h = rect[1]   # width height
            angle = rect[2] # angle
            box = cv2.boxPoints(rect)
            box = np.int0(box)
            
            # if h>w
            if h>w :
                angle -= 90
            
            
            #print('%s    center:(%d,%d),w:%d,h:%d,angle:%d\n'%(name,x,y,w,h,angle))
            #contour_info = '%s    center:(%d,%d),w:%d,h:%d,angle:%d\n'%(name,x,y,w,h,angle)
            #f_feature.write(contour_info)
            
            cv2.drawContours(img,[box], -1, (0, 255, 0), 1) #@ -1(draw all contour) 1:thickness
            #cv2.drawContours(img_gray,[box],-1,(0,0,255),2)
            #cv2.imwrite('test.png', img)
            
            mask = np.zeros(img.shape, dtype="uint8")  # define contour mask
            cv2.drawContours(mask, [box], -1, (255,255,255), -1) # in mask is white
            object_img = and_images(img_gray,mask)     # denoise, get object
            #cv2.imwrite('mask.png', mask)
            
        
            rows, cols = img.shape[:2]
            length = max(rows,cols)*2
            #@ transfer and rotation the object
            s_x = length/2 - x
            s_y = length/2 - y
            print(s_x,s_y)
            H = np.float32([[1,0,s_x],[0,1,s_y]])
            img2 = cv2.warpAffine(object_img, H, (length, length))
            M = cv2.getRotationMatrix2D((length/2, length/2), angle, 1)
            img3 = cv2.warpAffine(img2, M, (length, length))
            
#            name_png2 = outpath + 'denoise_' + name
#            cv2.imwrite(name_png2,img3)
            
            #@ change center to the buttom (rotate 180)
            #get no rotate object
            _2, contours2, hierarchy2 = cv2.findContours(img3,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
            cnt_f = contours2[0]
            for cnt2 in range(len(contours2)):
                rect2 = cv2.minAreaRect(contours2[cnt2])
                w2, h2 = rect2[1]   # width height
                print('123 ',w2,h2)
                if w2*h2 > h*w/2:
                    cnt_f = contours2[cnt2]
                    break
            
            #cv2.imshow('img3', img3)
            #cv2.waitKey(0)
            x2, y2 = rect2[0]  # center
            w2, h2 = rect2[1]   # width height
                
            if h2 > w2: # find long side is width
                t = w2
                w2 = h2
                h2 = t
            
            # print('%s    center:(%d,%d),w:%d,h:%d,length/2:%d\n'%(name,x2,y2,w2,h2,length/2))
#            
            left_x = x2-w2/2
            left_y = y2-h2/2
            rotate180 = horizontal_weight(img3,left_x,left_y,h2,w2)
            if rotate180 == True:
                rotate180 = 180
            else:
                rotate180 = 0
            
            print(rotate180)
                        
            M = cv2.getRotationMatrix2D((x2, y2), rotate180, 1.0)
            cos = np.abs(M[0, 0])
            sin = np.abs(M[0, 1])
         
            # compute the new bounding dimensions of the image
            nW = int((h2 * sin) + (w2 * cos))
            nH = int((h2 * cos) + (w2 * sin))
         
            # adjust the rotation matrix to take into account translation
            M[0, 2] += (nW / 2) - x2
            M[1, 2] += (nH / 2) - y2
         
            # perform the actual rotation and return the image
            finded = True
            img4 = cv2.warpAffine(img3, M, (nW, nH))
            #cv2.imshow('img4', img4)
            #cv2.waitKey(0)
            return cv2.warpAffine(img3, M, (nW, nH))
 
def feature_extract3(inimg):
    
    # binary input image
    inimg_gray = cv2.cvtColor(inimg, cv2.COLOR_BGR2GRAY)
    ret,img_thd = cv2.threshold(inimg_gray,127,255,cv2.THRESH_BINARY)# binary the gray image
    img_thd = 255-img_thd
    
    img = cv2.cvtColor(img_thd, cv2.COLOR_GRAY2BGR)
    height, width = img.shape[:2]
    # rgb to gray
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    
    #find contour   
    finded = False
    _, contours, hierarchy = cv2.findContours(img_gray,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
    for cnt in range(len(contours)):
        #cnt = contours[0]
        rect = cv2.minAreaRect(contours[cnt])
        x, y = rect[0]  # center
        w,h = rect[1]   # width height
        
        contour_size = w*h
        image_size = width*height
        
        print(image_size, contour_size)
        
        if contour_size > image_size/16 and finded == False:
            x,y = rect[0]  # center
            w,h = rect[1]   # width height
            angle = rect[2] # angle
            box = cv2.boxPoints(rect)
            box = np.int0(box)
            
            # if h>w
            if h>w :
                angle -= 90
            
            
            #print('%s    center:(%d,%d),w:%d,h:%d,angle:%d\n'%(name,x,y,w,h,angle))
            #contour_info = '%s    center:(%d,%d),w:%d,h:%d,angle:%d\n'%(name,x,y,w,h,angle)
            #f_feature.write(contour_info)
            
            cv2.drawContours(img,[box], -1, (0, 255, 0), 1) #@ -1(draw all contour) 1:thickness
            #cv2.drawContours(img_gray,[box],-1,(0,0,255),2)
            #cv2.imwrite('test.png', img)
            
            mask = np.zeros(img.shape, dtype="uint8")  # define contour mask
            cv2.drawContours(mask, [box], -1, (255,255,255), -1) # in mask is white
            object_img = and_images(img_gray,mask)     # denoise, get object
            #cv2.imwrite('mask.png', mask)
            
            #####	
            rows, cols = img.shape[:2]
            length = max(rows,cols)*2
            s_x = length/2 - x
            s_y = length/2 - y
            H = np.float32([[1,0,s_x],[0,1,s_y]])
            img2 = cv2.warpAffine(object_img, H, (length, length))
            	    
            img3 = rotate_img(img2, length/2, length/2, length, length, angle)   
            #cv2.imshow('img3', img3)
            #cv2.waitKey(0)
            #####
           
            #@ change center to the buttom (rotate 180)
            #get no rotate object
            _2, contours2, hierarchy2 = cv2.findContours(img3,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
            cnt_f = contours2[0]
            for cnt2 in range(len(contours2)):
                rect2 = cv2.minAreaRect(contours2[cnt2])
                w2, h2 = rect2[1]   # width height
                print('123 ',w2,h2)
                if w2*h2 > h*w/2:
                    cnt_f = contours2[cnt2]
                    break
            
            #cv2.imshow('img3', img3)
            #cv2.waitKey(0)
            x2, y2 = rect2[0]  # center
            w2, h2 = rect2[1]   # width height
                
            if h2 > w2: # find long side is width
                t = w2
                w2 = h2
                h2 = t
            
            # print('%s    center:(%d,%d),w:%d,h:%d,length/2:%d\n'%(name,x2,y2,w2,h2,length/2))
#            
            left_x = x2-w2/2
            left_y = y2-h2/2
            rotate180 = horizontal_weight(img3,left_x,left_y,h2,w2)
            if rotate180 == True:
                rotate180 = 180
            else:
                rotate180 = 0
            
            print(rotate180)
            
            img4 = rotate_img(img3, x2, y2, w2, h2, rotate180)            
         
            # perform the actual rotation and return the image
            finded = True
            #img4 = cv2.warpAffine(img3, M, (nW, nH))
            #cv2.imshow('img4', img4)
            #cv2.waitKey(0)
            return img4          
